...
export const getTodoList = () => (dispatch) => fetchUtil('/tasks', 'GET', null)
...
export const postTodoList = (body) => (dispatch) => fetchUtil(`/tasks`, 'POST', JSON.stringify(body))
    .then(dispatch(setLoading()))
    .then((data) => data.json())
    .then((response) => {
        if (!response.message) {
...
export const deleteTodo = (id) => (dispatch) => fetchUtil(`/tasks/${id}`, 'DELETE', null)
    .then((response) => response.json())
    .then((data) => dispatch(deleteTodoSuccess(data)))
    .catch((error) => dispatch(deleteTodoError(error)));

export const completeTodo = (id, completed) => (dispatch) => fetchUtil(`/tasks/${id}`, 'PATCH', JSON.stringify({ completed }))
    .then((response) => response.json())
